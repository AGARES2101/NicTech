...
export async function POST(request: NextRequest) {
  try {
    ...
    const revisorUrl = `${serverUrl}/rsapi/modules/fr/ondemand`
    ...
    if (!response.ok) {
      throw new Error(`Ошибка запроса распознавания лица: ${response.statusText}`)
    }
    ...
    console.error("Ошибка запроса распознавания лица:", error)
    return NextResponse.json(
      {
        success: false,
        message: "Ошибка запроса распознавания лица",
      },
      { status: 500 },
    )
  }
