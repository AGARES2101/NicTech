...
export async function POST(request: NextRequest) {
  try {
    ...
    const revisorUrl = `${serverUrl}/rsapi/modules/fr/checkfacepresence`
    ...
    if (!response.ok) {
      throw new Error(`Ошибка проверки соответствия лица: ${response.statusText}`)
    }
    ...
    console.error("Ошибка проверки соответствия лица:", error)
    return NextResponse.json(
      {
        success: false,
        message: "Ошибка проверки соответствия лица",
      },
      { status: 500 },
    )
  }
